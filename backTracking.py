...
n = 100
dnode = [[0 for _ in range(n)] for _ in range(n)]
dnode = [
...
def randomMatrix():
    for i in range(0, n):
        for j in range(0, n):
            if i != j:
                dnode[i][j] = random.randint(1, 10)
    print(dnode)


def backTracking(cnode, gnode, order):  # cnode当前所在结点  gnode已走结点 order次序（用于代表当前分支已经经过结点的数量）
    ...
    if order == n + 1:  # 满足条件代表已经回到出发点，这一分支结束，并回溯
        ...
        for i in range(n):  # 通过gnode得到的经过结点的步骤顺序根据dnode计算距离总和
        ...
        for i in range(1, n + 1):
            if gnode[i] == 0 and (i < n or i == order):  # 判断已走结点为0的部分代表还未走过， 附加条件保证不会提前执行回到原点的步骤
                backTracking(i, copy.deepcopy(gnode), order + 1)


# randomMatrix()
# exit(3)
start = datetime.now()
backTracking(0, copy.deepcopy(ngnode), 1)
end = datetime.now()
print("\n最优线路:路线：" + str(osquence) + "\t距离成本:" + str(ovalue))
print("花费时间：", (end - start).seconds, "秒")
